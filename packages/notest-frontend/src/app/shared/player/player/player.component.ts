...
import {
  CursorComponent,
  CursorDrawerComponent,
} from "./cursor-drawer.component";
...
export class PlayerComponent {
  ...
  private applyFrameEvents(events: BLEvent[]) {
    let lastChecked = events
      ...
      .reverse()
      .find(
        (e) => e.name == "checked" || e.name == "input"
      ) as BLInputCheckboxChangeEvent;
    let lastInput = events
      .slice()
      .reverse()
      .find(
        (e) => e.name == "value" || e.name == "input"
      ) as BLInputChangeEvent;
    if (lastInput) {
      let el = this.deserializedMap.get(
        lastInput.target as unknown as number
      ) as HTMLInputElement;
  ...
  private applyMutations(events: BLEvent[]) {
    ...
    for (let e of events)
      if ((e as BLDomEvent).mutations)
        mutations.push(...(e as BLDomEvent).mutations!);
    for (let m of mutations)
      new MutationRenderer().render(
        m,
  ...
  moveTo(timestamp) {
    ...
    if (!fullDom)
      throw new Error(
        "Full DOM event not found. Did you set the session events? (.setEvents)"
      );
    ...
    this.pause();
    let otherEvents = events.filter(
      (s) => s.name != dom.change && s.name != dom.full
    );
    this.currentTimestamp = timestamp;
    this.lastIndex = events.length - 1;
    new FocusHoverFakeClassAdderApi()
      .addHoverRules(this.target.doc)
      .then(() => {
  ...
  private playFrameEvents(msFromLast) {
    ...
    if (this.lastIndex < this.events.length - 1) {
      ...
      if (events.length > 0)
        this.currentTimestamp = events[events.length - 1].timestamp;
  ...
  private preventLinks() {
    this.target.doc.body
      .querySelectorAll("a:not(.bl_link_prevented)")
      .forEach((a) => {
