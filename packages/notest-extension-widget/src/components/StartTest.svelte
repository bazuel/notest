<script lang="ts">
  ...
  const imgUrl = 'https://notest.io/assets/img/test-img.svg';
  let imgBase64 = '';

  onMount(async () => {
    const url = `/media/asset-download?url=${imgUrl}`
...
{#if $appStore.logged}
  <button class="nt-button nt-start-registration-button" on:click={() => dispatcher('start-registration')}>Start
    Registration
  </button>
{:else}
  <div class="info-panel">
    {#if !createNewTestButtonPressed}
      <div class="nt-create-first-test-container">
        <img class="nt-create-first-test-img" src={'data:image/svg+xml;base64,' + imgBase64} alt="test-img"/>
        <p class="nt-p">To create your first test, press the button below</p>
        <button class="nt-button nt-create-first-test-button" on:click={() => createNewTestButtonPressed = true}>Create
          new test
        </button>
      </div>
    {:else}
      <div class="slide">
        ...
        <button class="nt-button nt-start-first-test-button" on:click={() => dispatcher('start-registration')}>Start
        </button>
      </div>
    {/if}
  </div>
