...
export function compareImage(image1: PNG, image2: PNG) {
  ...
  if (primaryColor1.primaryColor != primaryColor2.primaryColor) {
    console.log(`primary color are different: ${primaryColor1.primaryColor} vs ${primaryColor2.primaryColor}`);
    return {imageDiff: diff, pixelMismatch: 100};
  } else {
    console.log(`primary color are the same: ${primaryColor1.primaryColor} vs ${primaryColor2.primaryColor}`);
  }
