import { BLEvent, BLSessionEvent, eventReference } from '@notest/common';
...
export async function setupMonitor(
  ...
  let referenceObj: { reference: string } = { reference: '' };
  await context.exposeFunction('nt_collect', (ntEvent) => {
    if (eventsCollected.length == 0)
      referenceObj.reference = eventReference(ntEvent, Date.realNow ? Date.realNow() : Date.now());
    eventsCollected.push(ntEvent);
  });
