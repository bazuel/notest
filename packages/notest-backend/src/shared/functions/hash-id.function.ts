import type { NumberLike } from '../utils/hash-id.util';
import {
  fromAlphabet,
  isIntegerNumber,
  isPositiveAndFinite,
...
export default class Hashids {
  ...
  encode<T extends NumberLike | string>(first: T | T[], ...inputNumbers: T[]): string {
    const ret = '';
    ...
    if (!numbers.every(isIntegerNumber)) {
      numbers = numbers.map((n) =>
        typeof n === 'bigint' || typeof n === 'number' ? n : safeParseInt10(String(n))
      ) as T[];
    }
